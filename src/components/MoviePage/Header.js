  ...
  <header className="relative h-half">
     ...
     <div className="absolute bottom-0 w-full h-1/2 sliderGradient"></div>
      ...
      <ul className="mt-4 space-x-2 flex items-center">
        <li className="badge2">{original_language && getFullLanguageName(original_language)}</li>
        <li className="badge2">{release_date ? sliceYearFromDate(release_date) : first_air_date && sliceYearFromDate(first_air_date)}</li>
        <li className="badge2 text-yellow-500">{vote_average ? vote_average : <span className="text-sm">Not Released</span>}</li>
        <li className="badge2">{genres && genres.length && genres[0].name}</li>
