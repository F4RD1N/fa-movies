...
const Header = ({data}) => {
  ...
  <header className="relative h-header">
     ...
     <img src={backdrop_path ? MAIN_BACKDROP(backdrop_path) : MAIN_BACKDROP(poster_path)} className="w-full h-full object-cover object-top"/>
     
     <div className="absolute bottom-0 w-full h-1/2 sliderGradient"></div>
          
          
      <div className="absolute bottom-0 left-0 right-0 px-6 -translate-y-8 flex flex-col justify-center items-center">
      ...
      <ul className="mt-4 space-x-4 text-white">
