...
const SliderItem = ({ itemInfo }) => {
  ...
  return (
    <section className="block h-header">
      <div className="absolute z-20 bottom-0 w-full h-32 px-4 flex flex-col items-center">
        ...
        <ul className="mt-4 space-x-4 text-white flex items-center">
          ...
          <li className="badge text-3xl text-yellow-500">
            {vote_average && vote_average}
          </li>
          <li className="badge text-xs">
            {genre_ids && genre_ids.length && getGenresNameById(genre_ids[0])}
          </li>
          <li className="badge text-xs">
            +
            {genre_ids &&
              genre_ids.length &&
              getAgeLimit(getGenresNameById(genre_ids[0]))}
          </li>
        </ul>
      </div>
      <img
        src={MAIN_BACKDROP(backdrop_path)}
